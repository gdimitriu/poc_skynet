...
public class UpgradeServiceResource {
	...
	@POST
	@Path(IUpgradeServiceConstants.UPGRADE_REST_FILE_PATH + "/smallData")
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	public void postUpgradeServices(@FormDataParam(IUpgradeServiceConstants.UPGRADE_REST_NAME) String name,
			@FormDataParam(IUpgradeServiceConstants.UPGRADE_REST_XSD_SCHEMA) String upgradeDefinition,
			@FormDataParam(IUpgradeServiceConstants.UPGRADE_REST_BINDINGS) String upgradeBindings,
	...
	@POST
	@Path(IUpgradeServiceConstants.UPGRADE_REST_FILE_PATH + "/bigData")
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	public void postUpgradeServices(@FormDataParam(IUpgradeServiceConstants.UPGRADE_REST_NAME) String name,
			@FormDataParam(IUpgradeServiceConstants.UPGRADE_REST_XSD_SCHEMA) InputStream upgradeDefinition,
			@FormDataParam(IUpgradeServiceConstants.UPGRADE_REST_BINDINGS) InputStream upgradeBindings,
			@FormDataParam(IUpgradeServiceConstants.UPGRADE_REST_XML_DATA) InputStream upgradeData,
			@FormDataParam(IUpgradeServiceConstants.UPGRADE_REST_FORCE) Boolean force) {
		try {
			//get the context
			JAXBContext jxbContext = SingletonJaxbContextFactory.getSingletonFactory()
			...
			Unmarshaller unMarshaller = jxbContext.createUnmarshaller();
			SingletonServices.getSingleton().upgrade(unMarshaller, upgradeData);
			
		} catch (MalformedURLException | JAXBException e) {
			e.printStackTrace();
		}
		System.out.println(new BufferedReader(new InputStreamReader(upgradeDefinition))
