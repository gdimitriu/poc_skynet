...
import org.skynet.upgrade.resources.restAPI.IUpgradeServiceConstants;
...
public class UpgradeRestTest {
	...
	public void sendRestUpgradeSmallData() {
		...
		target.path(IUpgradeServiceConstants.UPGRADE_REST_PATH).path(IUpgradeServiceConstants.UPGRADE_REST_FILE_PATH).path("smallData")
	...
	public void sendRestUpgradeBigData() {
		Client client = ClientBuilder.newClient().register(MultiPartFeature.class).register(MultiPartWriter.class);
		WebTarget target = client.target(baseUri);
		@SuppressWarnings("resource")
		FormDataMultiPart multipartEntity = new FormDataMultiPart()
		...
		target.path(IUpgradeServiceConstants.UPGRADE_REST_PATH).path(IUpgradeServiceConstants.UPGRADE_REST_FILE_PATH).path("bigData")
		.request(new String[]{MediaType.MULTIPART_FORM_DATA}).post(Entity.entity(multipartEntity, multipartEntity.getMediaType()));
	...
	public static void main(String[] args) {
		...
		test.sendRestUpgradeSmallData();
		test.sendRestUpgradeBigData();
	}
